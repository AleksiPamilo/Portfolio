...
const NotFound: React.FC = () => {
    const { useDarkmode: isDarkmode } = useDarkmodeContext();
    ...
    return (
        <div className={`${isDarkmode ? "text-white" : "text-black"} flex w-screen h-screen absolute justify-center pt-[10rem] md:pt-[15rem]`}>
            <div>
                ...
                <Link to="/" className="py-4 px-5 text-xl rounded border-2 text-cyan-500 border-cyan-500 hover:bg-cyan-600 hover:text-white ease-in-out delay-75">Back to Home Page!</Link>
            </div>
        </div>
    )
}
